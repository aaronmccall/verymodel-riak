...
function getBucket(obj, args) {
    return args.bucket || obj.getBucket();
}

function build_get_request(obj, args) {
    var payload = { bucket:  getBucket(obj, args)};
...
function build_index_request(obj, args) {
    var argLen  = args.length,
        payload = { bucket: getBucket(obj, args), qtype: 0 };
...
function build_search_request(obj, args) {
    var argLen  = args.length,
        payload = { bucket: getBucket(obj, args) };
...
function build_mapreduce_request(obj, args) {
    ...
    if (payload.inputs && payload.inputs.index && !payload.inputs.bucket) {
        payload.inputs.bucket = getBucket(obj, args);
    }
    return payload;
}
