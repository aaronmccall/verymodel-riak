...
FilteredIndexStream.prototype._initDataStream = function () {
    this._model.getLogger().debug('FilteredIndexStream initializing data stream');
    var seenKeys = this._allStream ? this._allStream._keys : undefined;
    this._allStream = this._model._indexQuery.apply(this._model, this._search)
                          .pipe(new KeyToValueStream({model: this._model, keys: seenKeys}));
